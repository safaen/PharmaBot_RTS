#!/usr/bin/env python3
"""
pharmabot_env.py — Environnement Gymnasium PharmaBot avec connexion EDF
Équipe PharmaBot — Systèmes Embarqués Temps Réel 2025-2026
Prof. Khaoula Boukir — Ibn Tofaïl University

Modifications vs hospitalbot_env.py :
    - 4 départements nommés (Pharmacy, ICU, Emergency, Consultation)
    - Connexion EDF : subscriber /pharmabot/tache_courante change la cible
    - Récompense RT-aware selon la priorité (HARD/SOFT/FIRM)
    - Statistiques par département et niveau RT
    - Robot nommé PharmaBot
"""
import rclpy
...
DEPARTEMENTS = {
    "pharmacie":    {"nom": "Pharmacy_Room",    "x":  1.0, "y":  16.0, "couleur": "BLEU",   "priorite": None,      "deadline_sec": None},
    "reanimation":  {"nom": "ICU_Room",         "x": 11.0, "y":   5.5, "couleur": "ROUGE",  "priorite": "HARD_RT", "deadline_sec": 30},
    "urgences":     {"nom": "Emergency_Room",   "x": -5.0, "y":  -6.6, "couleur": "ORANGE", "priorite": "SOFT_RT", "deadline_sec": 120},
    "consultation": {"nom": "Consultation_Room","x": -2.0, "y": -27.0, "couleur": "VERT",   "priorite": "FIRM_RT", "deadline_sec": 300},
}
...
class PharmaBotEnv(RobotController, Env):
    ...
    def __init__(self):
        ...
        self._initial_agent_location = np.array([1.0, 16.0, -90], dtype=np.float32)
        self._departement_cible  = "reanimation"
        self._target_location    = np.array([DEPARTEMENTS["reanimation"]["x"], DEPARTEMENTS["reanimation"]["y"]], dtype=np.float32)
        self._priorite_courante  = "HARD_RT"
        self._deadline_sec       = 30
        self._temps_debut        = time.time()
        self._medicament_courant = "Médicament standard"

        # ── Connexion EDF : subscriber qui reçoit les missions du RT Scheduler ──
        self._sub_mission = self.create_subscription(
            String, "/pharmabot/tache_courante", self._cb_mission_edf, 10)
        ...
        if self._normalize_act:
            ...
            self.action_space = Box(low=np.array([self._min_linear_velocity, -self._angular_velocity]),
                                    high=np.array([self._max_linear_velocity, self._angular_velocity]), dtype=np.float32)
        ...
        self.robot_locations = [
            [1,16,-90,-1,1,-0.5,0.5,-30,30],[1,10,90,-3,3,-1,1,-30,30],
            [11,13,180,-1,1,-0.25,0.25,-30,30],[6.7,13,45,-0.1,0.1,-0.1,0.5,-15,15],
            [11.5,5.7,180,-0.2,0.2,-0.1,0.1,0,30],[7.5,4.8,0,-1,0.5,-0.1,0.1,-15,15],
            [7.7,-8,90,-0.5,0.5,-1.5,1.5,-30,30],[10,-2.1,180,-0.5,0.5,0,0,-10,10],
            [-2.3,-30.5,90,-0.2,0.5,-0.5,1.5,-20,45],[4,-27.4,180,-2,2,-1,0.7,-30,30],
            [-7.7,-30.8,90,-0.2,0.2,-0.5,1,-20,20],[-9.7,-26,-30,-1,0.2,-0.5,0.5,-20,20],
            [-2.1,-24.6,180,-1,1,-0.3,0.3,-45,30],[-5,-21,-90,-1,1,-0.5,2,-30,30],
            [-5,-6.6,90,-0.5,0.5,-1,1,-30,30],[-3.2,-2.9,180,-0.5,0,-1,0.2,-20,20],
            [-5,4,-90,-0.5,0.5,-1,1,-30,30],[-2.2,1,180,-0.5,2,-0.2,0.5,-30,30],
            [-3.6,10.9,180,-1,1.5,-1,1,-15,45],[-7.4,10.4,0,-0.1,1,-0.7,0.7,-30,30],
            [-1.6,-8.5,0,-1,1,-0.5,0.5,-30,30],[3.3,-8.6,180,-0.2,1,-0.5,0.5,-30,30],
            [5,-6,90,-0.8,0,-1,1,-45,45],[3.2,-2.8,-30,0,1,-1,0,-15,15],
            [2.8,-14.5,-90,0,0,-0.2,0,-15,15],[1.7,-19,90,0,0.5,-0.2,0.5,-15,15],
        ]
        self.target_locations = [
            [1,10,-3,3,-1,3],[1,16,-1,1,-1,1],[6.7,12,-0.1,0.1,-0.5,2],[11,13,-0.5,0.5,-0.2,0.2],
            [8,4.8,-1.5,1.5,-0.1,0.1],[11.3,5.5,-0.3,0.3,-0.2,0.2],[10.8,-2.1,-1,1,-0.1,0.1],
            [8.3,-6.8,-0.5,0.5,-1,1],[4.3,-27.6,-2,0.5,-0.5,0.5],[-2,-29.8,-0.5,0.5,-1,1],
            [-10.5,-26.3,-0.2,1,-1,1],[-7.7,-30,-0.8,0.8,-1,1],[-5,-21,-1,1,-2.5,0.5],
            [-1.5,-24.3,-1,1,-1,1],[-3.1,-3.5,-0.1,0.1,-1,1],[-5.2,-7,-0.9,1,-2,2],
            [0,2,-3,0,-1,1],[-4.5,4,-0.5,0.5,-1,1],[-7,10.3,-0.5,0.5,-0.5,0.5],
            [-2.6,10.3,-0.5,2,-2,2],[3.3,-8.6,-0.5,0.5,-0.5,0.5],[-1,-8.5,-1,0,-0.2,0.2],
            [3,-3.5,0,0,-1,1],[4.6,-6.4,-0.5,0.5,-1,1],[1.5,-19,-0.1,0.5,-1,1],[2.8,-15.4,0,0,-0.5,0.5],
        ]
        ...
        self.get_logger().info(f"Dept: {self._departement_cible.upper()} | {self._priorite_courante} | D={self._deadline_sec}s")

    # ── Callback EDF : reçoit la mission du RT Scheduler ─────────────────────
    def _cb_mission_edf(self, msg: String):
        """
        Reçoit une mission depuis le EDF Scheduler et change la cible du robot.
        C'est ici que se fait la connexion EDF ↔ robot.
        """
        try:
            data = json.loads(msg.data)
            dept        = data.get("departement", "reanimation")
            type_rt     = data.get("type_rt", "HARD_RT")
            deadline    = data.get("deadline_sec", 30)
            medicament  = data.get("medicament", "Médicament standard")

            if dept not in DEPARTEMENTS or dept == "pharmacie":
                return

            # Mettre à jour la cible
            self._departement_cible  = dept
            self._priorite_courante  = type_rt
            self._deadline_sec       = deadline
            self._medicament_courant = medicament
            ...
            # Mettre à jour la cible dans Gazebo
            if self._visualize_target:
                self.call_set_target_state_service(self._target_location)

            self.get_logger().info(
                f"[EDF → ROBOT] Nouvelle cible : {DEPARTEMENTS[dept]['nom']} | "
                f"{type_rt} | D={deadline}s | {medicament}"
            )
        except (json.JSONDecodeError, Exception) as e:
            self.get_logger().error(f"Erreur mission EDF : {e}")
    ...
    def _choisir_departement(self):
        """Choisit aléatoirement le département selon distribution réaliste."""
    ...
    def compute_rewards(self, info):
        ...
        if self._reward_method == 0:
            if info["distance"] < self._minimum_dist_from_target:   reward = 1
            elif any(info["laser"] < self._minimum_dist_from_obstacles): reward = -1
            else: reward = 0
        elif self._reward_method == 1:
            if info["distance"] < self._minimum_dist_from_target:   reward = 1
            elif any(info["laser"] < self._minimum_dist_from_obstacles): reward = -0.1
            else: reward = 0
            ...
            if info["distance"] < self._minimum_dist_from_target:
                ...
                ar  = self._attraction_factor/info["distance"] if info["distance"] < self._attraction_threshold else 0
            ...
            if info["distance"] < self._minimum_dist_from_target:
                if self._priorite_courante == "FIRM_RT" and not deadline_ok:
                    ...
                    # Confirmer la livraison au DMA pipeline
                    msg = String()
                    msg.data = json.dumps({
                        "departement": self._departement_cible,
                        "type_rt": self._priorite_courante,
                        "medicament": self._medicament_courant,
                        "temps": temps_ecoule,
                    ...
                    self.get_logger().info(
                        f"[{self._priorite_courante}] ✓ LIVRAISON → "
                ...
                if not deadline_ok:
                    if self._priorite_courante == "HARD_RT":
                        ...
                        self.get_logger().info("[HARD RT] COLLISION + DEADLINE MANQUÉE")
    ...
    def _get_info(self):
        ...
        return {
            ...
            "deadline_respectee": (self._deadline_sec is None or t <= self._deadline_sec),
        }
